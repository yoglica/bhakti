...
// Get a page by slug (for about, contact, privacy, terms, etc.)
export async function getPageBySlug(slug) {
  ...
  try {
    const filePath = path.join(pagesDirectory, `${slug}.html`)
    ...
    return {
      ...
      imageHeight: metadata.image_height || 630,
      canonical: metadata.canonical || '',
      ogType: metadata.og_type || 'website',
      ogSiteName: metadata.og_site_name || '',
      twitterCard: metadata.twitter_card || 'summary_large_image',
      twitterSite: metadata.twitter_site || '',
      twitterCreator: metadata.twitter_creator || '',
      readingTime: metadata.reading_time || 3,
...
export async function getAllPages(forceRefresh = false) {
  ...
  if (!forceRefresh && cachedPages && (now - lastCacheTime) < CACHE_TTL) {
    return cachedPages
  }
  ...
  try {
    ...
    cachedPages = pages.sort((a, b) => new Date(b.date) - new Date(a.date))
    lastCacheTime = now
